...
Every Wallex user who sideloads the APK on Android 13+ (and especially on HyperOS / POCO devices) hits Android's `ACCESS_RESTRICTED_SETTINGS` gate when they try to enable the notification listener. The toggle is grayed out, the flow cannot auto-advance, and users have to discover an undocumented 8-step workaround (App Info -> kebab menu -> "Allow restricted settings") on their own. Until Wallex ships on Play Store, this gate blocks the auto-import core feature for 100% of sideloaded installs.
...
- Detection via a new `isRestrictedSettingsAllowed` method on the existing `com.wallex.capture/quirks` MethodChannel, using `AppOpsManager.unsafeCheckOpNoThrow("android:access_restricted_settings", ...)` with a try/catch fail-open.
...
| `android/app/src/main/kotlin/com/wallex/app/DeviceQuirksChannel.kt` | Modified | New `isRestrictedSettingsAllowed` MethodChannel branch (~15 LOC). |
...
None. Uses existing `device_info_plus`, `package_info_plus`, the existing `com.wallex.capture/quirks` channel, and the existing `WidgetsBindingObserver` pattern. No new packages, no new manifest entries, no new permissions.
...
- [ ] Tapping the primary CTA deep-links to App Info; returning to Wallex after enabling "Allow restricted settings" auto-advances within one resume cycle.
